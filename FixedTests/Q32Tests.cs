...
namespace Cuni.Arithmetics.FixedPoint.Tests
{
    ...
    public class FixedToStringTestClass
    {
        ...
        public void ToStringTest()
        {
            ...
            Assert.AreEqual(Math.Round(resultFixed, FixedTestsClass.roundDeximalCount).ToString(),
                Math.Round((a / (double)b), FixedTestsClass.roundDeximalCount).ToString());

        }
    }
    [TestClass()]
    public class FixedTestsClass
    {
        ...
        public void Test_All_Operations<T>(IReadOnlyCollection<(int,int)> TestData) where T : IArithmetics, new()
        {
            ...
            {
                Add<T>(data.Item1, data.Item2);
                Divide<T>(data.Item1, data.Item2);
                Subtract<T>(data.Item1, data.Item2);
                Multiply<T>(data.Item1, data.Item2);
            }
        }        
